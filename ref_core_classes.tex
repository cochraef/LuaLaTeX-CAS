...
\def\currref{Core!Classes}

\newcommand{\coderef}[3]{%
\index{\currref!\texttt{#3}}%
...
\coderef{function SymbolExpression:new(string)}{return SymbolExpression}{SymbolExpression}
...
\texttt{SymbolExpression}s have only one attribute: \texttt{symbol}. In the example above, the string \mintinline{lua}{"bar"} is stored in \mintinline{lua}{foo.bar}. 
...
\newcoderef{function BinaryOperation:new(operation, expressions)}{return BinaryOperation}{operation function, expressions table<number,Expression>}{BinaryOperation}
...
\newcoderef{function FunctionExpression:new(name,expressions)}{return FunctionExpression}{name string|SymbolExpression, expressions table<number,Expression>}{FunctionExpression}
...
\end{codebox}
The variable \texttt{name} can be a string (like above), or another \texttt{SymbolExpression}. But in this case, the variable \texttt{name} just takes the value of the string \mintinline{lua}{SymbolExpression.symbol}. The variable \texttt{expressions} must be a table of \texttt{Expression}s index by Lua numbers.
...
\end{codebox}
The attribute \texttt{derivatives} is a table of \texttt{Integer}s index by Lua numbers whose length equals \mintinline{lua}{#o.variables}. The default value for this table is a table of zeros. So for the example above, we have:
\begin{codebox}
    \begin{minted}{lua}
