...
function IntegralExpression.rationalfunction(expression, symbol)
    ...
    do
        ...
        if f == Integer.one() and g.degree == Integer(2) and disc < Integer.zero() then
            return Integer(2) * ARCTAN((Integer(2)*g.coefficients[2]*symbol+g.coefficients[1]) / sqrt(Integer(4)*g.coefficients[0]*g.coefficients[2]-g.coefficients[1] ^ Integer(2))) / sqrt(Integer(4)*g.coefficients[0]*g.coefficients[2]-g.coefficients[1] ^ Integer(2))
        end
    end
