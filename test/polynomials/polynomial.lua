...
testeq(h:squarefreefactorization():autosimplify(),  "(2 + (3 * x) + (x ^ 2))", h)
testeq(i:squarefreefactorization():autosimplify(), "((1 + x) * ((2 + x) ^ 3))", i)
testeq((Integer(2)*i):squarefreefactorization():autosimplify(), "((2 + (2 * x)) * ((2 + x) ^ 3))", (Integer(2)*i))
testeq(j:squarefreefactorization():autosimplify(), "((1 + x) * ((2 + x) ^ 2) * ((3 + x) ^ 3))", j)
testeq(o:squarefreefactorization():autosimplify(), "(24 + (50 * x) + (59 * (x ^ 2)) + (60 * (x ^ 3)) + (36 * (x ^ 4)) + (10 * (x ^ 5)) + (x ^ 6))", o)
endtest()

starttest("polynomial factorization")
testeq(c:factor(), "(4 * (1x^1+3x^0 ^ 1))", c)
testeq(h:factor():autosimplify(), "((1 + x) * (2 + x))", h)
testeq(k:factor():autosimplify(), "((5 + x) * (6 + x))", k)
testeq(j:factor():autosimplify(), "((1 + x) * ((2 + x) ^ 2) * ((3 + x) ^ 3))", j)
testeq(p:factor():autosimplify(), "((-24 + x) * (96 + (x ^ 2)) * (48 + (-24 * x) + (x ^ 2)))", p)
testeq(l:factor():autosimplify(), "((1 + z) * (2 + z) * (3 + z))", l)
testeq(m:factor():autosimplify(), "((1 + x) ^ 10)", m)
testeq(b:factor(), "(1/12 * (24x^2+1x^1+4x^0 ^ 1))", b)
testeq(o:factor():autosimplify(), "((1 + x) * (2 + x) * (3 + x) * (4 + x) * (1 + (x ^ 2)))", o)
testeq(n:factor():autosimplify(), "((1 + x) * (2 + x) * (3 + x) * (4 + x) * (1 + (x ^ 2)) * (1 + (-1 * (x ^ 2)) + (x ^ 4)))", n)
endtest()
