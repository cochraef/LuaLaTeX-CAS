...
testeq(a:expand(), "((x * y) + (x * z))", a)
testeq(b:expand(), "(x + (x * z))", b)
testeq(c:expand(), "((30 * y) + (6 * x * y) + (36 * (x ^ 2) * y) + (-5 * z) + (-1 * x * z) + (-6 * (x ^ 2) * z))", c)
testeq(d:expand(), "(6 + (11 * x) + (6 * (x ^ 2)) + (x ^ 3))")
endtest()

starttest("expression factoring beyond monovariate polynomials")
testeq(e:factor(), "(x * (x + (y * z)))")
testeq(f:factor(), "((x ^ -2) * (1 + x) * (1 + (-1 * x) + (x ^ 2)))")
testeq(g:factor(), "((e ^ x) * (1 + x) * (1 + (-1 * x)))")
