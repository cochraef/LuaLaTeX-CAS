...
\begin{CAS}
  vars('x')
  f = IntegralExpression.table(cos(x),x)
  g = IntegralExpression.table(x*cos(x),x)
\end{CAS}
...
\begin{CAS}
    ...
    g = IntegralExpression.substitutionmethod(x*sin(x),x)
\end{CAS}
\[ f = \print*{f} \qquad g = \print*{g}.\] 
\end{minted}
\tcblower
...
In some cases, better results are returned via {\ttfamily partialfractions}. For example:
\begin{codebox}
    \begin{minted}[fontsize=\small]{latex}
\begin{CAS}
  vars('x')
...
\begin{CAS}
    vars('x')
    f = x*sin(x)
    g = IntegralExpression.partsmethod(f,x)
    ff = sin(x^2)
    gg = IntegralExpression.partsmethod(ff,x)
\end{CAS}
\[ \int \print{f}\ dx = \print*{g} \] 
\[ \int \print{ff}\ dx = \print*{gg} \] 
\end{minted}
\tcblower
\begin{CAS}
    vars('x')
