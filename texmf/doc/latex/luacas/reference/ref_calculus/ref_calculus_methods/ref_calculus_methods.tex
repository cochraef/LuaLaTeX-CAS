...
\newcoderef{function IntegralExpression.table(IntegralExpression)}{return Expression|nil}{}
...
\begin{CAS}
  vars('x')
  f = int(cos(x),x)
  f = IntegralExpression.table(f)
  g = int(x*cos(x),x)
  g = IntegralExpression.table(g)
\end{CAS}
...
\newcoderef{function IntegralExpression.linearproperties(IntegralExpression)}{return Expression|nil}{}
...
\begin{CAS}
  vars('x')
  f = int(sin(x) + e^x,x)
  g = IntegralExpression.table(f)
  f = IntegralExpression.linearproperties(f)
\end{CAS}
\[ f = \print*{f} \qquad g = \print*{g} \]
\end{minted}
\tcblower
...
\newcoderef{function IntegralExpression.substitutionmethod(IntegralExpression)}{return Expression|nil}{} 
...
\begin{CAS}
  ...
  f = int(x*e^(x^2),x)
  g = int(x*e^x,x)
  f = IntegralExpression.substitutionmethod(f)
  g = IntegralExpression.substitutionmethod(g)
\end{CAS}
\[ f = \print*{f} \qquad g = \print*{g}.\] 
\end{minted}
\tcblower
\begin{CAS}
    vars('x')
    f = int(x*e^(x^2),x)
    f.enhancedsubstitution = 0
    g = int(x*e^x,x)
    g.enhancedsubstitution = 0
    f = IntegralExpression.substitutionmethod(f)
    g = IntegralExpression.substitutionmethod(g)
\end{CAS}
\[ f = \print*{f} \qquad g = \print*{g}.\] 
...
\begin{CAS}
    ...
    f = f:autosimplify()
    g = IntegralExpression.rationalfunction(int(f,x))
\end{CAS}
...
\begin{CAS}
  ...
  f = int(f,x)
  f = IntegralExpression.rationalfunction(f)
\end{CAS}
\[ \print{simplify(g)} \] 
\end{minted}
\tcblower
...
Attempts to integrate \texttt{integrand} with respect to \texttt{symbol} via \emph{integration by parts}; returns nil if no suitable application of IBP is found. For example:
...
Attempts to integrate \texttt{integrand} with respect to \texttt{symbol} by using the Euler formulas: 
...
\begin{CAS}
  ...
  f = e^x*sin(x)
  ff = x^2
  g = IntegralExpression.eulersformula(f,x)
  gg = IntegralExpression.eulersformula(ff,x)
\end{CAS}
\[ \print*{g} \] 
\[ \print*{gg}\]
\end{minted}
