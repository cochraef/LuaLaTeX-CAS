...
\begin{codebox}[sidebyside align=top]
    \begin{multicols}{2}{\footnotesize\inputminted[
    firstline=56,
    lastline=82,
    breaklines]
    {latex}
...
Alternatively, one could appeal directly to the \mintinline{lua}{tostring()} function in \texttt{luacas} and iterate over tables like \texttt{r} using Lua itself. This can often be a simpler solution, and it is exactly what Bob does in his complete project below. 
...
To check the type of an \texttt{Expression}, Charlie finds the \mintinline{latex}{\whatis} command to be helpful:
