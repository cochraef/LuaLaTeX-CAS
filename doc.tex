...
At any rate, Charlie now knows, according to \texttt{luacas}, that \texttt{q} is of the \texttt{Expression}-type \whatis{q}. The actual operator that's used to form \texttt{q} is stored in the attribute \mintinline{lua}{q.operation}. Although Charlie can't get this printed, like the \texttt{Expression} type, they can check the value with an \texttt{if-then} statement:
