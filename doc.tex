...
This section is geared towards for those who are interested in the inner design of the Lua end of the CAS. In particular, anyone who wishes to modify or expand the CAS for their own purposes should read this.
...
Every object in the CAS is an expression, meaning it inherits from the {\ttfamily Expression} class. Since the {\ttfamily Expression} class itself cannot be instantiated, it it closer to an interface in Java OOP terms. \footnote{In reality, interfaces are unnecessary in Lua due to its weak typing - Lua doesn't check whether an object has a method at compile time. The {\ttfamily Expression} class is really an abstract class in Java terms, but we will call it an interface for simplicity.} {\ttfamily Expression}s can store any number of other expressions as sub-expressions, depending on type. Expressions that
\subsubsection{Metatable inheritance}
